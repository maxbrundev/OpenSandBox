...
bool OpenSandBox::Resources::Parsers::AssimpParser::LoadModel(const std::string& p_filePath, Model& p_model, bool p_isLoadingMaterial)
{
	...
	ProcessNode(&identity, scene->mRootNode, scene, p_model.GetMeshes(), p_model.GetMaterials(), p_isLoadingMaterial);
...
void OpenSandBox::Resources::Parsers::AssimpParser::ProcessNode(const aiMatrix4x4* p_transform, const aiNode* p_node, const aiScene* p_scene, std::vector<Mesh*>& p_meshes, std::array<Material*, 255>& p_materials, bool p_isLoadingMaterial)
{
	...
	{
		ProcessNode(&nodeTransformation, p_node->mChildren[i], p_scene, p_meshes, p_materials, p_isLoadingMaterial);
	}
}

void OpenSandBox::Resources::Parsers::AssimpParser::ProcessMesh(const aiMatrix4x4* p_transform, const aiMesh* p_mesh, const aiScene* p_scene, std::vector<Geometry::Vertex>& p_outVertices, std::vector<uint32_t>& p_outIndices)
{
...
void OpenSandBox::Resources::Parsers::AssimpParser::ProcessMaterial(const aiMesh* p_mesh, const aiScene* p_scene, Material& p_outMaterial)
{
...
void OpenSandBox::Resources::Parsers::AssimpParser::LoadTexturesFromMaterial(const aiMaterial* p_mat, aiTextureType p_type, ETextureType p_textureType, std::unordered_map<ETextureType, std::shared_ptr<Texture>>& p_outTextures)
{
